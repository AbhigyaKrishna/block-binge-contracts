...
contract StreamingPlatform is IStreamingPlatform {
    ...
    event BillTime(address indexed user, address indexed creator, uint256 contentId, uint256 indexed time);
    ...
    function startVerification() external {
        emit VerifyPayment(msg.sender);
    }

    function billTime(uint256 contentId, uint256 time) external {
        emit BillTime(msg.sender, contentId, time);
    }
} 
